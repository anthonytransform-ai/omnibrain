...
| `有甚麼需要我決定？` | 顯示需要你決定的事項。 |
| `整理本次進度。` | 記錄目前進度。 |
| `保留為專案知識。` | 保留經你確認的長期專案知識。 |
| `標記為已完成。` | 在決定已解決後標記目前工作為完成。 |
| `封存工作。` | 按你的指示把目前工作移到封存區。 |
| `檢查 OmniBrain。` | 執行不會修改檔案的 OmniBrain 檢查。 |
| `更新 OmniBrain。` | 在保留你內容的前提下安全更新框架。 |

詳細行為由 `Vault/Core_OS/Runtime/Entry.md` 強制定義。這些繁體中文 Quick Calls 是標準例子，不是必須逐字輸入的指令。英文 Quick Calls 和其他同等意思的自然語言請求也可以使用，但中文讀者可以直接使用上表，不需要先改成英文。
...
當你準備結束某個工作項目時，說 `整理本次進度。` OmniBrain 應記錄目前位置、說明檢查了甚麼、列出任何不確定或受阻的事項，並詢問是否有內容應保留為長期專案知識。

若你回答「否」，請把它保留為工作紀錄。若你回答「是」，說 `保留為專案知識。` OmniBrain 應更新所屬的 System 或 Feature 筆記，保留既有專案知識並加以補充，必要時更新索引。

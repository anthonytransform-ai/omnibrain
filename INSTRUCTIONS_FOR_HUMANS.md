...
OmniBrain 內建了一個專家**子代理團隊**（如代碼審查員、架構師和 UI 設計師）。
- **觸發子代理**：只需告訴你的 AI *「審查我的代碼 (Review my code)」*、*「審計架構 (Audit the architecture)」* 或 *「設計 UI (Design the UI)」*。你的 AI 將自動生成一個子代理來提供幫助。
- **安全重構協議**：當你想對代碼庫進行大幅更改時，請要求你的 AI 運行安全重構協議。子代理被嚴格禁止編寫應用程式代碼——它們充當安全網。它們會在主 AI 應用更改*之前*起草計劃並審查代碼差異，確保你的項目永遠不會崩潰！
